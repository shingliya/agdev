...
void Application::Init()
{
	...
	Luala la("lua.lua");
	m_window_width = la.getInt("width");
	m_window_height = la.getInt("height");
...
void Application::Run()
{
	...
	while (!glfwWindowShouldClose(m_window) && /*!IsKeyPressed(VK_ESCAPE)*/ !exitbool)
	{
...
void Application::mouseMode(bool show)
{
	if (show)
	{
		glfwSetInputMode(m_window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
		MouseWithinScreen = false;
	}
		
	else
	{
		glfwSetInputMode(m_window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
		MouseWithinScreen = true;
		mouse_last_x = mouse_current_x;
		mouse_last_y = mouse_current_y;
	}
