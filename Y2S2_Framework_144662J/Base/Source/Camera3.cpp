...
float m_fTPVCameraOffset = 11.00f;
...
void Camera3::Init(const Vector3& pos, const Vector3& target, const Vector3& up, Player* player)
{
	this->position = defaultPosition = pos;
	this->target = defaultTarget = target;
	Vector3 view = (target - position).Normalized();
...
void Camera3::UpdatePostion(double dt)
{
	if (player->Zoom)
	{
		if (m_fTPVCameraOffset > 6.f)
			m_fTPVCameraOffset -= dt * (m_fTPVCameraOffset - 6) * 7;
		if (m_rightOffset < 1.5)
			m_rightOffset += dt * (1.5 - m_rightOffset) * 7;
		if (m_rightOffset > 1.5)
			m_rightOffset -= dt * (m_rightOffset - 1.5) * 7;
		if (m_upOffset > 1.5)
			m_upOffset -= dt * (m_upOffset - 1.5) * 7;
	}
	else if(player->state == player->Left || player->state == player->ForwardLeft || player->state == player->BackwardLeft)
	{
		if (m_rightOffset > -3)
		{
			m_rightOffset -= dt * (m_rightOffset - -3) * 3;
			if(m_rightOffset < -3)
				m_rightOffset = -3;
		}
		if (m_fTPVCameraOffset < 11.f)
		{
			m_fTPVCameraOffset += dt * (11 - m_fTPVCameraOffset) * 3;
			if(m_fTPVCameraOffset > 11.f)
				m_fTPVCameraOffset = 11;
		}
		if (m_upOffset < 2)
		{
			m_upOffset += dt * (2 - m_upOffset) * 7;
			if(m_upOffset > 2)
				m_upOffset = 2;
		}
	}
	else if(player->state == player->Right || player->state == player->ForwardRight || player->state == player->BackwardRight)
	{
		if (m_rightOffset < 3)
		{
			m_rightOffset += dt * (3 - m_rightOffset) * 3;
			if(m_rightOffset > 3)
				m_rightOffset = 3;
		}
		if (m_fTPVCameraOffset < 11.f)
	...
	else
	{
		if (m_fTPVCameraOffset < 11.f)
		{
			m_fTPVCameraOffset += dt * (11 - m_fTPVCameraOffset) * 7;
			if(m_fTPVCameraOffset > 11.f)
				m_fTPVCameraOffset = 11.f;
		}
		if (m_rightOffset > 0 || m_rightOffset < 0)
		{
			if(m_rightOffset > 0)
			{
				m_rightOffset -= dt * (m_rightOffset - 0) * 7;
				if(m_rightOffset < 0)
					m_rightOffset = 0;
			}
			else
			{
				m_rightOffset += dt * (0 - m_rightOffset) * 7;
				if(m_rightOffset > 0)
					m_rightOffset = 0;
			}
		}
	...
	position = player->GetPosition() - player->GetDirection() * m_fTPVCameraOffset;
	position += player->GetRight() * m_rightOffset;
	position.y += m_upOffset;
	target = player->GetPosition();
	target += player->GetRight() * m_rightOffset;
	target += player->GetDirection() * 200;
	up = player->GetUp();
}